...
function fillSchedule(table) {
    ...
    Object.keys(groups).forEach((id) => {
      ...
      Object.keys(weekdays).forEach((dayKey) => {
        ...
        for (let i = parseDay["y"]; i < parseDay["y"] + 15; i += 3) {
          schedule[groups[id]][weekdays[dayKey]] =
            schedule[groups[id]][weekdays[dayKey]] || {};
          if (
            table[transformCordsToId(x, i)] ||
            onIncludes(transformCordsToId(x, i))
          ) {
            const mainCellId =
              onIncludes(transformCordsToId(x, i)) || transformCordsToId(x, i);
            const mainCell1Id =
              onIncludes(transformCordsToId(x, i + 1)) ||
              transformCordsToId(x, i + 1);
            const mainCell2Id =
              onIncludes(transformCordsToId(x, i + 2)) ||
  ...
  function onIncludes(cell) {
    ...
    const res = merges.filter(
      (el) =>
        el["s"].c <= x && el["s"].r <= y && el["e"].c >= x && el["e"].r >= y
    );
    return res.length
      ? transformCordsToId(res[0]["s"]["c"], res[0]["s"]["r"])
      : false;
  }
  return schedule;
}
...
function transformIdToCords(id) {
  ...
  id.split("").forEach((el) =>
    isNaN(+el) ? (x += letterNumberInAlphabet(el)) : (y += el)
  );
...
function ABToNum(AB) {
  return AB.split("").reduce((acc, current, index) => {
    return (
      acc + letterNumberInAlphabet(current) * 26 ** (AB.length - index - 1)
    );
  }, 0);
}
